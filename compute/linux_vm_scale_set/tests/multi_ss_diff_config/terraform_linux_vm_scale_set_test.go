...
func TestTerraformLinuxVM(t *testing.T) {
	...
	test_structure.RunTestStage(t, "validate", func() {
		...
		scalesetNameList := terraform.OutputList(t, terraformOptions, "virtual_machine_linux_scale_set_name")
		resourceGroupName := terraform.Output(t, terraformOptions, "scaleset_resource_group")

		ssValue := removeCharacters(scalesetNameList[0], "[]")

		scaleSet := ListScaleSet(t, resourceGroupName, subscriptionID)
		expectedSsCount := 2
		assert.Equal(t, expectedSsCount, len(scaleSet))
		assert.Contains(t, scaleSet, ssValue, "Does not exists")

		for _, v := range scalesetNameList {
			ssValue := removeCharacters(v, "[]")
			ssInfo, err := GetVMSSInfo(subscriptionID, resourceGroupName, ssValue)
			if err != nil {
				fmt.Println("No oinfo received")
			}
