...
func TestTerraformNatGateway(t *testing.T) {
	...
	test_structure.RunTestStage(t, "validate", func() {
		...
		fePortsList := ListFePorts(t, resourceGroupName, subscriptionID, appGwsName)
		expectedFePCount := 2
		assert.Equal(t, expectedFePCount, len(fePortsList))
		assert.Contains(t, fePortsList, frontendPort, "Does not exists")
...
func ListFePorts(t *testing.T, resourceGroupName string, subscriptionID string, appGwsName string) []string {
	feports, err := ListFePortsE(resourceGroupName, subscriptionID, appGwsName)
	require.NoError(t, err)
	return feports
}

func ListFePortsE(resourceGroupName string, subscriptionID string, appGwsName string) ([]string, error) {
	var fePortsList []string
	ctx, cancel := context.WithTimeout(context.Background(), 6000*time.Second)
	defer cancel()
	...
	appGwInfo, err := appGwsClient.Get(ctx, resourceGroupName, appGwsName)
	if err != nil {
		return nil, err
	}

	for _, v := range *appGwInfo.FrontendPorts {
		fePortsList = append(fePortsList, *v.Name)
	}
	return fePortsList, nil
}
